...
set "ot=1.7.4"&set "wAction=Configure"&set "ou=DllExport"&set "ov=tools/net.r_eg.DllExport.Wizard.targets"&set "ow=packages"&set "ox=https://www.nuget.org/api/v2/package/"&set "oy=build_info.txt"&set "oz=!oo!"&set "wRootPath=%~dp0"&set /a wDxpOpt=0&set "oa="&set "ob="&set "oc="&set "od="&set "oe="&set "of="&set "og="&set "oh="&set "oi="&set "oj="&set /a ok=0&(if not defined op (if defined wAction goto pr
...
set oe=!v!&goto pw)else if [!om!]==[-eng] (chcp 437 >nul&goto pw)else if [!om!]==[-GetNuTool] (call :py -GetNuTool 10&set /a ok=!ERRORLEVEL! & goto pu)else if [!om!]==[-hMSBuild] (set oj=1
call :py -hMSBuild 9&set /a ok=!ERRORLEVEL! & goto pu)else if [!om!]==[-version] (@echo 1.7.4  %__dxp_pv%
goto pu)else if [!om!]==[-build-info] (set ob=1&goto pw)else if [!om!]==[-tests] (set /a "ol+=1" & call :px pp[!ol!] v
...
set o1=-GetNuTool "!on!" /p:ngserver="!ox!" /p:ngpath="!ow!" /p:proxycfg="!oh!"&call :pb o1 "no"))&(if defined oe ("!wPkgPath!\\tools\\PeViewer.exe" -list -pemodule "!oe!"
set /a ok=%ERRORLEVEL%&goto pu))&(if defined ob (call :pz "buildInfo = " wPkgPath oy
...
set o6=!o5:~%o4%!&(if defined oj (call :pb o6)else (set o6=-GetNuTool !o6!
...
set hl=!v!&call :i8 "selected vswhere version:" v&set hv=1&goto i7)else if [!hf!]==[-version] (echo 2.4.1.29880+57a01fe&goto i4)else if [!hf!]==[-priority] (set /a "he+=1" & call :jh ic[!he!] v
