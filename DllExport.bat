...
set "ot=1.7.4"&set "wAction=Configure"&set "ou=DllExport"&set "ov=tools/net.r_eg.DllExport.Wizard.targets"&set "ow=packages"&set "ox=https://www.nuget.org/api/v2/package/"&set "oy=build_info.txt"&set "oz=!oo!"&set "wRootPath=!cd!"&set /a wDxpOpt=0&set "oa="&set "ob="&set "oc="&set "od="&set "oe="&set "of="&set "og="&set "oh="&set "oi="&set "oj="&set /a ok=0&(if not defined op (if defined wAction goto pq
goto pr))&set or=!or:/?=/h!&call :inita po or pp&goto ps
:pr
echo.&echo .NET DllExport 1.7.4&echo Copyright (c) 2009-2015  Robert Giesecke&echo Copyright (c) 2016-2024  Denis Kuzmin ^<x-3F@outlook.com^> github/3F&echo.&echo MIT License&echo https://github.com/3F/DllExport&echo.&echo.&echo Usage: DllExport [keys] or built-in [-GetNuTool ... or -hMSBuild ...]&echo.&echo Keys&echo ----&echo -action {type} - Specified action for Wizard. Where {type}:&echo   * Configure - To configure DllExport for specific projects.&echo   * Update    - To update pkg reference for already configured projects.&echo   * Restore   - To restore configured DllExport.&echo   * Export    - To export configured projects data.&echo   * Recover   - To re-configure projects via predefined/exported data.&echo   * Unset     - To unset all data from specified projects.&echo   * Upgrade   - Aggregates an Update action with additions for upgrading.&echo.&echo -sln-dir {path}    - Path to directory with .sln files to be processed.&echo -sln-file {path}   - Optional predefined .sln file to be processed.&echo -metalib {path}    - Relative path to meta library.&echo -metacor {path}    - Relative path to meta core library.&echo -dxp-target {path} - Relative path to entrypoint wrapper of the main core.&echo -dxp-version {num} - Specific version of %~n0. Where {num}:&echo   * Versions: 1.7.4 ...&echo   * Keywords:&echo     `actual` - Unspecified local/latest remote version;&echo                ( Only if you know what you are doing )&echo.&echo -msb {path}           - Full path to specific MSBuild Tools.&echo -hMSBuild {args}      - Access to hMSBuild (built-in) https://github.com/3F/hMSBuild&echo -packages {path}      - A common directory for packages.&echo -server {url}         - Url for searching remote packages.
echo -proxy {cfg}          - To use proxy. The format: [usr[:pwd]@]host[:port]&echo -pkg-link {uri}       - Direct link to package from the source via specified URI.&echo -force                - Aggressive behavior, e.g. like removing pkg when updating.&echo -no-mgr               - Do not use %~n0 for automatic restore the remote package.&echo -mgr-up               - Updates %~n0 to version from '-dxp-version'.&echo -wz-target {path}     - Relative path to entrypoint wrapper of the main wizard.&echo -pe-exp-list {module} - To list all available exports from PE32/PE32+ module.&echo -eng                  - Try to use english language for all build messages.&echo -GetNuTool {args}     - Access to GetNuTool (built-in) https://github.com/3F/GetNuTool&echo -debug                - To show additional information.&echo -version              - Displays version for which (together with) it was compiled.&echo -build-info           - Displays actual build information from selected %~n0.&echo -help                 - Displays this help. Aliases: -help -h /h -? /?&echo.&echo Flags&echo -----&echo  __p_call - To use the call-type logic when invoking %~n0&echo.&echo Samples&echo -------&echo %~n0 -action Configure -force -pkg-link https://host/v1.7.4.nupkg&echo %~n0 -action Restore -sln-file "Conari.sln"&echo %~n0 -proxy guest:1234@10.0.2.15:7428 -action Configure&echo %~n0 -pe-exp-list bin\Debug\regXwild.dll&echo.&echo %~n0 -mgr-up -dxp-version 1.7.4&echo %~n0 -action Upgrade -dxp-version 1.7.4&echo.&echo %~n0 -GetNuTool "Conari;regXwild;Fnv1a128"&echo %~n0 -hMSBuild ~x ~c Release&echo %~n0 -GetNuTool vsSolutionBuildEvent/1.16.1:../SDK ^& SDK\GUI
goto pt
:ps
set /a ol=0
:pu
set om=!po[%ol%]!&(if [!om!]==[-help] (goto pr)else if [!om!]==[-h] (goto pr)else if [!om!]==[-?] (goto pr)else if [!om!]==[/h] (goto pr ))&(if [!om!]==[-debug] (set oa=1
goto pv)else if [!om!]==[-action] (set /a "ol+=1" & call :pw po[!ol!] v
set wAction=!v!&(for %%g in (Restore, Configure, Update, Export, Recover, Unset, Upgrade, Default) do (if "!v!"=="%%g" goto pv))&echo Unknown -action !v!&exit/B1)else if [!om!]==[-sln-dir] (set /a "ol+=1" & call :pw po[!ol!] v
set wSlnDir=!v!&goto pv)else if [!om!]==[-sln-file] (set /a "ol+=1" & call :pw po[!ol!] v
set wSlnFile=!v!&goto pv)else if [!om!]==[-metalib] (set /a "ol+=1" & call :pw po[!ol!] v
set wMetaLib=!v!&goto pv)else if [!om!]==[-metacor] (set /a "ol+=1" & call :pw po[!ol!] v
set wMetaCor=!v!&goto pv)else if [!om!]==[-dxp-target] (set /a "ol+=1" & call :pw po[!ol!] v
set wDxpTarget=!v!&goto pv)else if [!om!]==[-dxp-version] (set /a "ol+=1" & call :pw po[!ol!] v
set ot=!v!&goto pv)else if [!om!]==[-msb] (set /a "ol+=1" & call :pw po[!ol!] v
set oc=!v!&goto pv)else if [!om!]==[-packages] (set /a "ol+=1" & call :pw po[!ol!] v
set ow=!v!&goto pv)else if [!om!]==[-server] (set /a "ol+=1" & call :pw po[!ol!] v
set ox=!v!&goto pv)else if [!om!]==[-proxy] (set /a "ol+=1" & call :pw po[!ol!] v
set oh=!v!&set wProxy=!v!&goto pv)else if [!om!]==[-pkg-link] (set /a "ol+=1" & call :pw po[!ol!] v
set od=!v!&set ot=!om!&goto pv)else if [!om!]==[-force] (set of=1&goto pv)else if [!om!]==[-no-mgr] (set /a wDxpOpt^|=1
goto pv)else if [!om!]==[-mgr-up] (set og=1&goto pv)else if [!om!]==[-wz-target] (set /a "ol+=1" & call :pw po[!ol!] v
set ov=!v!&goto pv)else if [!om!]==[-pe-exp-list] (set /a "ol+=1" & call :pw po[!ol!] v
set oe=!v!&goto pv)else if [!om!]==[-eng] (chcp 437 >nul&goto pv)else if [!om!]==[-GetNuTool] (call :px -GetNuTool 10&set /a ok=!ERRORLEVEL! & goto pt)else if [!om!]==[-hMSBuild] (set oj=1
call :px -hMSBuild 9&set /a ok=!ERRORLEVEL! & goto pt)else if [!om!]==[-version] (@echo 1.7.4  %__dxp_pv%
goto pt)else if [!om!]==[-build-info] (set ob=1&goto pv)else if [!om!]==[-tests] (set /a "ol+=1" & call :pw po[!ol!] v
set oi=!v!&goto pv)else (echo Incorrect key: !om!&set /a ok=1&goto pt))
:pv
set /a "ol+=1" & if %ol% LSS !pp! goto pu
:pq
call :py "dxpName = " ou&call :py "dxpVersion = " ot&call :py "-sln-dir = " wSlnDir&call :py "-sln-file = " wSlnFile&call :py "-metalib = " wMetaLib&call :py "-metacor = " wMetaCor&call :py "-dxp-target = " wDxpTarget&call :py "-wz-target = " ov&call :py "#opt " wDxpOpt&(if defined ot ((if "!ot!"=="actual" (set "ot="))))
set wPkgVer=!ot!&(if z%wAction%==zUpgrade (call :py "Upgrade is on"
set og=1&set of=1))&call :pz ow&set "ow=!ow!\\"&set "on=!ou!"&set "wPkgPath=!ow!!ou!"&(if defined ot (set "on=!on!/!ot!"
set "wPkgPath=!wPkgPath!.!ot!"))&(if defined of ((if exist "!wPkgPath!" (call :py "Removing the old version. '-force' key rule. " wPkgPath
rmdir /S/Q "!wPkgPath!"))))&set o0="!wPkgPath!\\!ov!"&call :py "wPkgPath = " wPkgPath&(if not exist !o0! ((if exist "!wPkgPath!" (call :py "Trying to replace obsolete version ... " wPkgPath
rmdir /S/Q "!wPkgPath!"))&call :py "-pkg-link = " od&call :py "-server = " ox&(if defined od (set ox=!od!
(if "!ox::=!"=="!ox!" (set ox=!cd!/!ox!))&(if "!wPkgPath::=!"=="!wPkgPath!" (set "on=:../!wPkgPath!")else (set "on=:!wPkgPath!"))))&(if defined oc (set msb.gnt.cmd=!oc!))
set o1=-GetNuTool "!on!" /p:ngserver="!ox!" /p:ngpath="!ow!" /p:proxycfg="!oh!"&call :pa o1 "no"))&(if defined oe ("!wPkgPath!\\tools\\PeViewer.exe" -list -pemodule "!oe!"
set /a ok=%ERRORLEVEL%&goto pt))&(if defined ob (call :py "buildInfo = " wPkgPath oy
(if not exist "!wPkgPath!\\!oy!" (echo information about build is not available.
set /a ok=2&goto pt))&type "!wPkgPath!\\!oy!"
goto pt))&call :py "wRootPath = " wRootPath&call :py "wAction = " wAction&call :py "wMgrArgs = " wMgrArgs&call :py "wzTarget = " o0&(if not defined oi ((if not exist !o0! (echo Target cannot be initialized. Try to use another keys.
set /a ok=2&goto pt))&(if defined oc (call :py "Use specific MSBuild Tools: " oc
(if not exist "!oc!" (echo MSBuild Tools was not found. Check -msb key.>&2
set /a ok=2&goto pt))&call "!oc!" !o0! /nologo /v:m /m:4)else (set o2=~x -cs !o0! /nologo&call :pa o2))))&(if !ERRORLEVEL! NEQ 0 (echo Something went wrong. Use `-debug` key for details.
set /a ok=2))
:pt
(if defined oi (echo Running Tests ... "!oi!"
set o2=~x -cs "!oi!" /nologo&call :pa o2&exit/B!ERRORLEVEL!))&(if defined og ((copy /B/Y "!wPkgPath!\\DllExport.bat" "!oz!" > nul) && ( echo Manager has been updated. & exit/B0 ) || ( (echo -mgr-up failed:!ok! 1>&2) & exit/B1 )))
exit/B!ok!
:pw
call :eva %*&exit/B
:px
set om=%~1&set /a o3=%~2&call :py "accessing to !om! ..."
(for /L %%p in (0,1,8181)do (if "!or:~%%p,%o3%!"=="!om!" (set o4=!or:~%%p!
set o5=!o4:~%o3%!&(if defined oj (call :pa o5)else (set o5=-GetNuTool !o5!
call :pa o5))&exit/B!ERRORLEVEL!)))&call :py "!om! is corrupted: " or&exit/B1
:pa
set o6=!%~1!&if not defined logo set "logo=%~2"
call :py "invoke a built-in hMSBuild: " o6 logo&(if defined oj (if defined oa set o6=-debug !o6!))
call :pb !o6!&exit/B!ERRORLEVEL!
:py
(if defined oa (set "o7=%~1" & echo [ %TIME% ] !o7! !%2! !%3!))
exit/B0
:pz
call :pc %1&call :pd %1&exit/B0
:pc
call :pe %1 "-=1"&exit/B0
:pd
call :pe %1 "+=1"&exit/B0
:pe
set o8=z!%1!z&(if "%~2"=="-=1" (set "o9=1")else (set "o9="))&(if defined o9 (set /a "i=-2")else (set /a "i=1"))
:pf
(if "!o8:~%i%,1!"==" " (set /a "i%~2"
goto pf))&if defined o9 set /a "i+=1"
(if defined o9 (set "%1=!o8:~1,%i%!")else (set "%1=!o8:~%i%,-1!"))
exit/B0
:pb
setlocal disableDelayedExpansion&set _kH=%~n0 -hMSBuild
::! hMSBuild 2.4.1.22899+e7cd595
...
set hl=!v!&call :i8 "selected vswhere version:" v&set hv=1&goto i7)else if [!hf!]==[-version] (echo 2.4.1.22899+e7cd595&goto i4)else if [!hf!]==[-priority] (set /a "he+=1" & call :jh ic[!he!] v
